...
var rules = [
    ...
    },
    {
        ...
        regex: function regex(match) {
            return new RegExp(
                ';' + match[1] + '=' + defaultOrConstrained(match[2])
            )
        }
    },
    {
...
var parseQueryParams = searchParams.parse
        ...
        },
        {
            ...
            value: function test(path, opts) {
                ...
                var options = _extends(
                    { strictTrailingSlash: false, queryParams: {} },
                    opts
                )
                // trailingSlash: falsy => non optional, truthy => optional
                var source = optTrailingSlash(
                    this.source,
                    options.strictTrailingSlash
                )
                // Check if exact match
                var matched = this._urlTest(
                    path,
                ...
                var queryParams = parseQueryParams(path, options.queryParams)
                var unexpectedQueryParams = Object.keys(
                    queryParams
                ).filter(function(p) {
                    return !_this2._isQueryParam(p)
                })
            ...
            value: function build() {
                ...
                var encodedUrlParams = Object.keys(params)
                    ...
                    .reduce(function(acc, key) {
                        ...
                        var val = params[key]
                        var encode = _this4._isQueryParam(key)
                            ? identity
                            : encodeURI
                ...
                if (
                    this.urlParams.some(function(p) {
                ...
                var base = this.tokens
                    ...
                    .map(function(t) {
                        if (t.type === 'url-parameter-matrix')
                            return (
                                ';' + t.val + '=' + encodedUrlParams[t.val[0]]
                            )
                        return /^url-parameter/.test(t.type)
                ...
                var searchPart = searchParams.build(
                    searchParams$$1,
                    options.queryParams
                )
